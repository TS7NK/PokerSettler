...
struct GameSetupView: View {
    ...
    var body: some View {
        VStack {
            Text("Game Setup")
                .font(.largeTitle)
                .padding(.top, 50) // Move the title more up
                .padding(.bottom, 20) // Add padding between the title and the first row
            
            ForEach([1, 5, 10, 50, 100], id: \.self) { denomination in
                HStack {
                    Image("chip\(denomination)")
                        .resizable()
                        .frame(width: 60, height: 60) // Enlarge the chip icon
                        .padding(.trailing, 10)
                    
                    TextField("Value", value: $chipValues[denomination], formatter: NumberFormatter())
                        .textFieldStyle(RoundedBorderTextFieldStyle())
                        .keyboardType(.decimalPad)
                        .frame(width: 150, height: 40) // Enlarge the entry box
                        .padding(.leading, 10)
                    
                    Spacer() // Pushes the content to the left
                }
                .padding([.leading, .trailing], 20) // Add padding to the sides
                .padding(.bottom, 10) // Add padding between rows
            }
